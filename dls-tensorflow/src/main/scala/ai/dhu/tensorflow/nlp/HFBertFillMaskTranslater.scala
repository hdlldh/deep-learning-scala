...
class HFBertFillMaskTranslator extends Translator[String, String] {
  ...
  override def processInput(ctx: TranslatorContext, input: String): NDList = {
    ...
    val indices = tokenList.map(r => vocabulary.getIndex(r).toInt)
    val attentionMask = token.getAttentionMask.map(i => i.toInt)
    attentionMask.foreach(println(_))
...
//
//      (1 to TopK).map { i =>
//        val out = ndArray.get(maskIndex).argSort().getInt(len - i)
